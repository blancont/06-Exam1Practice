...
Authors: David Mutchler, Vibha Alangar, Valerie Galluzzi, Mark Hays,
         Amanda Stouder, their colleagues and Nathaniel Blanco.
...
def problem2a(circle, rectangle, window):
    ...
    # still need to make line an arrow
...
def problem2b(rect, n, delta, win):
    ...
    for k in range(n):
        rec = rg.Rectangle(rg.Point(corner1.x - (delta * k), corner1.y - (delta * k)),
                           rg.Point(corner2.x + (delta * k), corner2.y + (delta * k)))
        rec.attach_to(win)
        win.render()
